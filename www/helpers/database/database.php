<?php
    ...
    class UsersTable {
         ...
         *         False otherwise
         */
        public static function checkUsernameExists($username, $db) {
        ...
        public static function addUser($username, $password, $firstName, $lastName, $email, $hash, 
            ...
            $id = $db->getLastId();
            $db->execute('INSERT INTO profile(userId) VALUES (:id)', 
                array(':id' => $id));
            return $id;
        }
         ...
         * @return integer            1 is update succeeded, 0 if failed
         */
        public static function updateProfileInfo($id, $firstName, $middleName, $lastName, $gender, 
    ...
    class FriendsTable extends DatabaseHelper {
        ...
        public function getFriends($userID) {
            $friends = array();
            $result = $this->fetch("SELECT login 
        ...
        public function addFriend($requester, $addFriend) {
            ...
            if(!UsersTable::checkUsernameExists($addFriend, $this))
                throw new DatabaseException('Invalid username');

            // Check if friend already exists
            $result = $this->fetch("SELECT * 
                FROM friendships as f, users as u
                WHERE ((f.user1=:requester AND f.user2=u.ID) OR (f.user2=:requester AND f.user1=u.ID)) 
                AND u.login=:addFriend AND status=1",
                Array(':requester' => $requester, ':addFriend' => $addFriend));

            if(sizeof($result) != 0)
                throw new DatabaseException("Already friends with $addFriend");
            ...
            if(sizeof($result) != 0)
                throw new DatabaseException("Friend request already sent to $addFriend");
            ...
            if(sizeof($result) != 0) {
                $result = $this->execute("UPDATE friendships
                    SET status=1
            ...
            // Add request
            $result = $this->execute("INSERT INTO friendships(user1, user2, startTimestamp)
        ...
        public function deleteFriend($user1, $user2) {
            $this->execute("DELETE FROM friendships 
